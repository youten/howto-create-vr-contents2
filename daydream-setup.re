...
とくにこのホームボタンの長押しによる中心のリセット（@<img>{daydream-tutorial}-左下）は重要操作なので覚えておきましょう。
...
== デバッグ設定とVR Entry Flow

この節では、Daydreamのデベロッパー向けのデバッグ設定を用いて、不要な操作を省略したり、Daydream ViewがなくともDaydreamアプリの利用や開発を可能にする方法を紹介します。
...
また、後者を組み合わせると、@<kw>{Daydream Viewを持っていなくともDaydreamアプリの利用や開発を行うことが可能になります}。

デベロッパー向けの設定については公式サイトの「The Controller Emulator@<fn>{controller-emulator}」のページに記載されているが、手順のみがシンプルに掲載されており、少しわかりづらいです。
もう少し噛み砕いて手順を確認していきましょう。

前節のDaydreamの初期セットアップの最後、コントローラのペア設定画面（@<img>{daydream-setup-01}-右）まで進めると、実は初期セットアップとしてはコントローラの設定を残して完了しているため、Daydreamアプリを終了して、再度起動するとホーム画面が表示されます。

//image[daydream-developer-settings-01][デベロッパー向け設定の有効化][scale=1.00]

ホーム画面の左上のハンバーガーメニュー（≡）（@<img>{daydream-developer-settings-01}-左）から設定画面（@<img>{daydream-developer-settings-01}-中）を呼び出し、「ビルド バージョン」を7回タップすると、「デベロッパー向けの設定」隠しメニューが表示されるようになります（@<img>{daydream-developer-settings-01}-右）。
...
Daydream母艦とあるAndroidデバイスとは別のAndroidデバイスを用意してください。
...
「Skip VR entry screens」設定が有効になっていると、本物のコントローラのペアリングも、ホームボタンの押下も要求されません。

二眼ステレオのVRモードになったら、もう一台のAndroidで「Controller Emulator」アプリを起動します。
エミュレータアプリの表示がConnected（Bluetooth）になったら、正常に動作しています。
うまくいかない際には、Bluetoothの設定や、アプリの起動順番に気をつけつつ、何度かやり直してみてください。
アプリ上にも表示されていますが、タッチパネル部のクリックのエミュレートには、アプリ上でのダブルタップで対応しています。
...
ただし、あくまでお試しで。
Daydreamの真の力はきちんとした環境でぜひ体験してみてください。
...
結論としては、Daydream互換機として動作はしましたが、格別安いというわけでもなく、あまりオススメはできない機種です。
日本特有の例のアレの話もありますし、おとなしく新型のDaydream Viewの発売を待つのが良さそうです。
